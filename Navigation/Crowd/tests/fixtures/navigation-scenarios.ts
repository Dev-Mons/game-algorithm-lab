...
// Engine regression geometry only; never registered in the app or measurement scripts.
const NAVIGATION_FIXTURES: readonly ScenarioDefinition[] = [
  ...SCENARIOS,
  {
    id: 'obstacle-field',
    name: 'Obstacle Field',
...
export function getTestScenario(id: string): ScenarioDefinition {
  const scenario = NAVIGATION_FIXTURES.find((entry) => entry.id === id);
  if (!scenario) throw new Error(`Unknown navigation fixture: ${id}`);
  return scenario;
}
