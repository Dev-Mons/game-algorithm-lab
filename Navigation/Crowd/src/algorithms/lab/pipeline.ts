...
import { LocalMotionSolver, retainedBodyDetourRadius } from '../lab-motion/local-motion';
...
const EMPTY_NAVIGATION_PATH: readonly Vec2[] = [];
...
export class LabPipeline {
  ...
  readonly navigationTargets: (Vec2 | undefined)[];
  readonly navigationPaths: (readonly Vec2[])[];
  ...
  constructor(readonly world: LabWorld, readonly options: ExperimentOptions, primary: FlowField,
    ...
    this.navigationTargets = new Array(count);
    this.navigationPaths = new Array(count).fill(EMPTY_NAVIGATION_PATH);
  ...
  private allocateSlots(previous?: LabPipeline): void {
    ...
    // Even when both parked bodies settle toward the same ingress lane, their
    // inflated detour polygons must leave room for the largest approaching body.
    // Gap belongs to BOTH sides of that lane; physical non-overlap alone is not
    // sufficient for ORCA or its conservative parked-body visibility graph.
    const settleTolerance = Math.max(0.25, maxRadius * 0.2);
    const spacing = retainedBodyDetourRadius(maxRadius, maxRadius, c.agentGap) * 2 + settleTolerance * 2;
    this.slotSpacing = spacing;
    const occupied = new Set<string>();
    const preserved = new Uint8Array(this.targets.length);
    if (previous && previous.slotSpacing !== spacing) {
      // Configuration objects are mutable. A command or terrain rebuild must
      // not carry forward a lattice that no longer provides the required lane.
      this.world.state.active.fill(1);
    } else if (previous) {
  ...
  sample(agent: number, x: number, y: number, out: Vec2): boolean {
    out.x = 0; out.y = 0;
    this.navigationTargets[agent] = undefined;
    this.navigationPaths[agent] = EMPTY_NAVIGATION_PATH;
    ...
    if (this.options.destination === 'slots' && grid.isSegmentSafe(x, y, goal.x, goal.y)) {
      this.navigationTargets[agent] = goal;
    ...
    // Local parked-body routing must reconnect to this real visible corridor
    // waypoint. The final slot can still be behind another static wall.
    this.navigationTargets[agent] = path[visible]!;
    this.navigationPaths[agent] = path;
  ...
  step(current: AgentBuffer, next: AgentBuffer, step: number, densityScale: number): CrowdMovementResult {
    ...
    const motion = this.local.solve({ current, next, preferredX: this.preferredX, preferredY: this.preferredY,
      ...
      navigationPaths: this.navigationPaths, navigationPathCursor: this.pathCursor,
      ...
      yieldWhenStalled: this.options.queue,
