...
export class LocalMotionSolver {
  ...
  private retainedBodyPreference(input: LocalMotionInput, i: number, px: number, py: number): { x: number; y: number } {
    ...
    while (path.length && Math.hypot(path[0]!.x - x, path[0]!.y - y) < Math.max(0.05, radii[i]! * 0.1)) path.shift();
    ...
    if (!path.length) {
      ...
      for (const j of parked) {
        const radius = (radii[i]! + radii[j]! + config.agentGap + 0.02) / Math.cos(Math.PI / 16);
