...
export class LabPipeline {
  ...
  sample(agent: number, x: number, y: number, out: Vec2): boolean {
    ...
    let cursor = Math.min(this.pathCursor[agent]!, Math.max(0, path.length - 1));
    if (!path.length) return false;
    while (cursor < path.length - 1 && Math.hypot(path[cursor]!.x - x, path[cursor]!.y - y) < this.world.config.navCellSize * 0.7) cursor += 1;
    let visible = -1;
  ...
  step(current: AgentBuffer, next: AgentBuffer, step: number, densityScale: number): CrowdMovementResult {
    ...
    for (let agent = 0; agent < current.count; agent += 1) {
      ...
      let speed = c.maxSpeed * clamp((distance - arrival * 0.5) / slowSpan, 0, 1);
