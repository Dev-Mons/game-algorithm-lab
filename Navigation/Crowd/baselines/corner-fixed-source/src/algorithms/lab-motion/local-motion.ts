...
export interface LocalMotionInput {
  ...
  navigationTargets?: readonly (Vec2 | undefined)[];
  navigationPaths?: readonly (readonly Vec2[])[];
  navigationPathCursor?: Int32Array;
  yieldWhenStalled?: boolean;
...
export class LocalMotionSolver {
  ...
  private detourTargets: (Vec2 | undefined)[] = [];
  private detourBestDistance = new Float64Array(0);
  private detourNoProgress = new Float64Array(0);
  private detourPriorityPeers: number[][] = [];
  ...
  private chooseVelocities(input: LocalMotionInput, maxRadius: number, metrics: LocalMotionMetrics): void {
    ...
    for (let i = 0; i < current.count; i++) {
      ...
      if (input.yieldWhenStalled && current.stalledFor[i]! >= config.stallSeconds) {
        // A common handed rotation gives opposing travelers opposite world-side
        // sidesteps. Apply it to the actual local route, not a discarded goal ray.
        const x = px; px = x * 0.8 - py * 0.3; py = py * 0.8 + x * 0.3;
      }
      ...
      if (this.detourPaths[i]!.length && !this.nearest.some(j => this.movable[j]) && allowed > 0) {
        // This local visibility route is holonomic. Carrying the previous leg's
        // heading through a tight vertex can send the unit into another aisle
        // and repeatedly reverse its route. Ramp speed, then let ORCA choose a
        // feasible velocity along the visible leg without angular momentum.
        const preferredSpeed = Math.hypot(px, py);
        const speed = Math.min(preferredSpeed, Math.hypot(current.vx[i]!, current.vy[i]!) + allowed);
        if (preferredSpeed > 1e-9) { px *= speed / preferredSpeed; py *= speed / preferredSpeed; }
      } else if (change > allowed && allowed > 0) {
      ...
      if (input.avoidance === 'orca') {
        ...
        const route = this.detourPaths[i]!;
        // A parked disc after an imminent, validated route turn is not on the
        // intended straight trajectory. Predict static bodies only to that turn;
        // moving peers keep the full reciprocal anticipation horizon.
        const staticHorizon = route.length > 1
          ? Math.min(horizon, Math.max(config.fixedDelta,
            Math.hypot(route[0]!.x - current.x[i]!, route[0]!.y - current.y[i]!) / Math.max(1e-9, Math.hypot(px, py))))
          : horizon;
        for (const j of this.nearest) {
          ...
          planes.push(discOrcaPlane(
            ...
            radii[i]! + radii[j]! + config.agentGap + 1e-5, current.active[j] === 0 ? staticHorizon : horizon, config.fixedDelta,
  ...
  private retainedBodyPreference(input: LocalMotionInput, i: number, px: number, py: number): { x: number; y: number } {
    ...
    const primaryTarget = input.navigationTargets?.[i] ?? input.targets?.[i];
    if (!primaryTarget) return { x: px, y: py };
    let target: Vec2 = primaryTarget;
    const priorTarget = this.detourTargets[i];
    const distanceToTarget = Math.hypot(target.x - current.x[i]!, target.y - current.y[i]!);
    if (!priorTarget || priorTarget.x !== target.x || priorTarget.y !== target.y) {
      path.length = 0; this.detourCooldown[i] = 0;
      this.detourPriorityPeers[i]!.length = 0;
      this.detourTargets[i] = { x: target.x, y: target.y };
      this.detourBestDistance[i] = distanceToTarget; this.detourNoProgress[i] = 0;
    }
    // Speed alone misses an orbit. Require meaningful new progress toward this
    // same route target; returning to a previously visited distance is not new
    // progress. Use the configured stall interval and the body's spatial scale.
    const progress = Math.min(radii[i]!, this.detourBestDistance[i]! * 0.5);
    if (distanceToTarget < this.detourBestDistance[i]! - progress) {
      this.detourBestDistance[i] = distanceToTarget; this.detourNoProgress[i] = 0;
    } else this.detourNoProgress[i] = this.detourNoProgress[i]! + config.fixedDelta;
    if (this.detourCooldown[i]! > 0) this.detourCooldown[i]!--;
    if (speed < 1e-9 || (!path.length && (this.detourCooldown[i]! > 0
      || (current.stalledFor[i]! < config.stallSeconds
        && (this.nearest.some(j => this.movable[j]) || this.detourNoProgress[i]! < config.stallSeconds))))) return { x: px, y: py };
    const x = current.x[i]!, y = current.y[i]!;
    const peers = this.detourPriorityPeers[i]!;
    // Only the lower-priority stalled traveler changes its route. These bodies
    // remain active reciprocal ORCA participants, never fixed physical bodies.
    // Retain the routing concession while the peer remains in the neighborhood;
    // its first successful movement must not immediately revoke the concession.
    for (let n = peers.length - 1; n >= 0; n--) {
      if (!this.nearest.includes(peers[n]!) || !this.movable[peers[n]!]) peers.splice(n, 1);
    }
    if (input.priorityRecovery !== false && current.stalledFor[i]! >= config.stallSeconds) {
      for (const j of this.nearest) {
        if (j < i && this.movable[j] && current.stalledFor[j]! >= config.stallSeconds && !peers.includes(j)) {
          peers.push(j); path.length = 0; this.detourCooldown[i] = 0;
        }
      }
    }
    const routeObstacles = this.nearest.filter(j => current.active[j] === 0 || peers.includes(j));
    if (!routeObstacles.length) { path.length = 0; return { x: px, y: py }; }
    if (!path.length && !routeObstacles.some(j => Number.isFinite(sweptDiscTime(
    ...
    const occupied = (point: Vec2): boolean => routeObstacles.some(j =>
      Math.hypot(point.x - current.x[j]!, point.y - current.y[j]!) < radii[i]! + radii[j]! + config.agentGap);
    if (occupied(target)) {
      // Static A* waypoints are guidance, not required parking coordinates. A
      // retained body can occupy one; reconnect to a real later corridor vertex
      // instead of demanding an impossible visit to that occupied point.
      const corridor = input.navigationPaths?.[i];
      const cursor = input.navigationPathCursor?.[i] ?? -1;
      const reconnect = corridor?.slice(cursor + 1).find(point => !occupied(point));
      if (!reconnect) return { x: px, y: py };
      target = reconnect;
    }
    ...
    const clear = (ax: number, ay: number, bx: number, by: number): boolean => {
      ...
      for (const j of this.nearest) {
        if (this.movable[j] && !peers.includes(j)) continue;
    ...
    if (clear(x, y, target.x, target.y)) {
      if (target.x === primaryTarget.x && target.y === primaryTarget.y) { path.length = 0; return { x: px, y: py }; }
      path = [{ x: target.x, y: target.y }]; this.detourPaths[i] = path;
    }
    if (path.length && !clear(x, y, path[0]!.x, path[0]!.y)) path.length = 0;
    if (path.length > 1 && !clear(path[0]!.x, path[0]!.y, path[1]!.x, path[1]!.y)) path.length = 0;
    if (!path.length) {
      ...
      for (const j of routeObstacles) {
        const radius = retainedBodyDetourRadius(radii[i]!, radii[j]!, config.agentGap);
        const addPoint = (x: number, y: number): void => {
          if (clear(x, y, x, y)
            && this.nearest.every(k => (this.movable[k] && !peers.includes(k))
              || Math.hypot(x - current.x[k]!, y - current.y[k]!) >= radii[i]! + radii[k]! + config.agentGap)) nodes.push({ x, y });
        };
        for (let side = 0; side < 16; side++) {
          const angle = side * Math.PI / 8;
          addPoint(current.x[j]! + Math.cos(angle) * radius, current.y[j]! + Math.sin(angle) * radius);
        }
        // A valid source/target may lie inside the polygon's conservative node
        // envelope beside an axis-aligned wall. Its adjacent polygon vertex can
        // be inside that wall, leaving no entry despite a safe wall-tangent path.
        // Exact axis/circle intersections connect that path without resolution tuning.
        for (const endpoint of [{ x, y }, target]) {
          const dx = endpoint.x - current.x[j]!, dy = endpoint.y - current.y[j]!;
          if (Math.abs(dx) < radius) {
            const offset = Math.sqrt(radius * radius - dx * dx);
            addPoint(endpoint.x, current.y[j]! - offset); addPoint(endpoint.x, current.y[j]! + offset);
          }
          if (Math.abs(dy) < radius) {
            const offset = Math.sqrt(radius * radius - dy * dy);
            addPoint(current.x[j]! - offset, endpoint.y); addPoint(current.x[j]! + offset, endpoint.y);
          }
        }
      }
  ...
  private ensureCapacity(count: number, config: SimulationConfig): void {
    if (this.bodies.length !== count) {
      ...
      this.detourTargets = new Array(count);
      this.detourBestDistance = new Float64Array(count); this.detourNoProgress = new Float64Array(count);
      this.detourPriorityPeers = Array.from({ length: count }, () => []);
