...
test('four refreshed styles render complete real meshes and export editable examples',async({page})=>{
  ...
  for(const style of ['shop','office','urban-shop','urban-office'] as const){
    const doc=createDocument(grid,17,style,undefined,undefined,scene);
    if(style.startsWith('urban'))doc.buildings[0].design.overrides={familyId:'bay-3',palette:style==='urban-shop'?'clay':'sage'};
  ...
  const cards=[['shop','상가형','석재 창턱 · 층간 몰딩 · 처마 장식'],['office','업무형','넓은 창 · 수직 금속 핀 · 슬림 난간'],['urban-shop','도시형 복합 상가','테라코타 프레임 · 2층 연속 유리'],['urban-office','도시형 업무','가는 커튼월 프레임 · 금속 창살']];
