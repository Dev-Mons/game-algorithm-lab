...
test("saved special rule renders and preserves theme independence through edit/history/JSON", async ({
  page,
}, info) => {
  const errors: string[] = [];
  ...
  const doc = setBuildingRule(createDocument(grid, 42, "shop"), "0,0,0", "parking");
  await page
    .locator("#file")
    .setInputFiles({
      name: "rule.json",
