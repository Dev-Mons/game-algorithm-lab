...
상태: 구현 전 연구 계획 · 기준일: 2026-09-16 · 대상: `Procedural/LevelGeneration`

이 문서는 향후 구현의 기준이며, 이번 변경은 문서 작성·검토만 수행한다. 코드, 패키지, 테스트, Asset은 아직 만들지 않는다. 아래의 정책과 수치는 별도 표시가 없으면 **초기 설계 선택 또는 검증할 가정**이지 구현 완료나 실측 결과가 아니다.
...
MVP는 아래 **6개 작업**으로 나눈다. 각 작업은 한 AI Coding Agent가 제한된 변경과 해당 검증을 끝낼 수 있는 단위다. 앞 단계의 계약/Fixture가 통과하지 않으면 다음 UI나 최적화 작업으로 넘어가지 않는다. 경로 예시는 모두 `Procedural/LevelGeneration/` 기준의 **미래 제안**이다. 현재 존재하는 것은 `.gitkeep`과 이 계획 문서뿐이며 예시 파일을 이번에 생성하지 않는다.
...
| 기본 전체 재생성이 초기 편집에 충분하다. | 지정 대표 입력의 단계별 p50/p95와 목표 차이 기록. | 6 이후 최적화 착수 여부 결정. 현재 실측 없음. |
...
확인한 저장소 기준은 main의 `9b5ee2954a3338f5e6e3360148c6b310e3e6d34e`다. 루트 `AGENTS.md`를 확인했으며 `Procedural` 및 대상 폴더에 별도 지침/기존 계획 문서는 없고 대상에는 `.gitkeep`만 있었다. Navigation 전용 불변조건·실행 명령은 이 프로젝트의 요구로 복사하지 않는다.

이번 변경 범위는 `Procedural/LevelGeneration/DEVELOPMENT_PLAN.md` 한 파일이다. 추가 문서 연결은 필수인 기존 목차가 없어 하지 않는다. 문서 구조/코드 펜스/표/경로, 데이터 계약과 기대값의 논리적 일관성, MVP/후속 경계, 하부면/공유면/공동 규칙, 미검증 가정 표시, 문서 외 변경 여부를 검토한다. 위 각 단계의 테스트는 **미래 구현 시 수행할 계획**이며 이번에 실행한 테스트가 아니다. 기존 프로젝트의 전체 테스트·빌드·성능 측정은 수행하지 않는다.
...
- 예상 변경 파일: `package.json`, 설치 결과의 lockfile, 최소 `tsconfig.json`, `src/core/generate.ts`, `tests/generate.test.ts`, `fixtures/single.json`, `fixtures/adjacent-x.json`, `fixtures/sealed-cavity.json`. 모두 대상 폴더 안의 향후 제안이며 이번 문서 작업에서는 생성하지 않는다. 별도 Vitest 설정 파일은 기본 실행으로 해결되지 않을 때만 추가한다.
- 필요한 테스트: 외피/Placement 정확한 집합 6/10/54개, 공유면/공동 0개, 하부면 존재, 여섯 방향의 position2/기저/Face coverage 일치, 반복 및 Cell 순서 변경 시 결과 동일. 테스트 코드와 기대 JSON은 구현 작업에서 작성한다.
...
- 다음 작업: MVP 2의 단차 Roof/Terrace 자동 정책, 높은 하부 노출면, 외부 Edge 특징을 기존 Fixture 체계에 추가한다. 이 첫 구현은 이번 문서 변경에서 시작하지 않는다.
