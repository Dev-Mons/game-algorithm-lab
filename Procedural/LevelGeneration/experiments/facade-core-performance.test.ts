...
it('measures full vertical and facade evaluation without cached rule results',()=>{
  const summary=(values:number[])=>{const sorted=[...values].sort((a,b)=>a-b);return {n:values.length,medianMs:sorted[Math.ceil(sorted.length*.5)-1],p95Ms:sorted[Math.ceil(sorted.length*.95)-1],maxMs:sorted.at(-1),meanMs:values.reduce((a,b)=>a+b,0)/values.length};};
  const rows=[];
  ...
  const report={measuredAt:new Date().toISOString(),runtime:process.version,description:'24x24x24, seed 17, immutable analysis/base outside timing, no entrance claims, no cache, 1 first + 40 repeated rule evaluations per style',summaryConvention:'Nearest-rank p50/p95: sorted[ceil(n*p)-1]; first sample excluded from repeated summaries',rows};
