...
- 오브젝트 설치 → 시설에서 외벽 영역을 지정한다. 깊이 1셀의 직사각형 U/V 영역을 지원한다. 한 높이의 가로 줄은 발코니, 여러 높이는 비상계단, 여러 높이가 지면 Y=0까지 이어지면 엘리베이터로 자동 판별한다. 인접 영역 추가·삭제 시 다시 판별한다.
- 기존 JSON의 명시적 시설 종류와 외벽 솔리드 변경 요청은 호환을 위해 유지한다. 새 편집은 자동 종류를 사용하며 기본 외벽 패턴을 유지한다. 시설은 모두 표시용이며 상층 portal이나 이동 경로를 만들지 않는다.
