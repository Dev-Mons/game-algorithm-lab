...
const root=process.cwd(),out=resolve(root,'artifacts/parking-reimplementation');
...
try{
 ...
 if(after.stalls<14)throw new Error('Single-hole layout lost its usable rows');
 await page.keyboard.press('Escape');await page.screenshot({path:resolve(out,'R12-single-hole.png')});
 ...
 for(const name of ['R30','L16','U16-arms6','rect-6x16']){await load(name);await page.screenshot({path:resolve(out,`${name}.png`)});}
 await load('R30');await page.locator('[data-camera="iso"]').click();await page.screenshot({path:resolve(out,'R30-iso.png')});
