...
const root=process.cwd(),output=resolve(root,'artifacts/parking-reimplementation');await mkdir(output,{recursive:true});
...
try{
  ...
  for(const {id,document} of cases){
    ...
    const components=areas.flatMap(a=>a.plans.map(p=>({key:p.circulation.componentKey,status:p.circulation.status,axis:p.circulation.axis,offset:p.circulation.offset,layout:p.circulation.layoutId,search:p.circulation.search,islands:p.islands,stalls:p.stalls.length,potential:p.quality.potentialStalls,reasonCodes:p.circulation.reasonCodes,gates:p.circulation.gates.map(g=>({opening:g.openingCells,heading:g.inwardHeading})),counters:p.circulation.counters,rejections:p.quality.primaryRejectionCounts,candidateReasons:Object.fromEntries([...new Set(p.circulation.traces.flatMap(t=>t.candidates.flatMap(c=>c.reasonCodes)))].map(code=>[code,p.circulation.traces.flatMap(t=>t.candidates).filter(c=>c.reasonCodes.includes(code)).length]))})));
    ...
    if(['R12','R30','L16','U16-arms6','rect-6x16','boundary','R12-minus-5-5'].includes(id))await writeFile(resolve(output,`${id}.json`),JSON.stringify(document));
  }
  ...
  await writeFile(resolve(output,'report.json'),JSON.stringify({baseCommit:'abb1d0318226b4913c78679db8c8008289ddf0c2',algorithm:'parking-circulation-v2',scope:'Single uncached sample per scenario; core time, not browser latency. No optimality or real vehicle-physics claim.',rows,editChanges,buildingSummary},null,2));
