...
import {completeFaceAsset} from '../src/core/complete-face-assets';
...
it('band openings and piers differ geometrically, plinth appears only on the base foot, and glass joints meet in four directions',()=>{
  ...
  for(const style of ['shop','office'] as const)for(const band of ['base','body','crown'] as const)for(const row of ['foot','repeat','head','single'] as const)for(const direction of ['PX','NX','PZ','NZ'] as const){
    ...
    const seam=(geometry:typeof leftGeometry,edge:number,matrix:Matrix4)=>{const p=geometry.getAttribute('position'),index=geometry.getIndex()!,points:string[]=[];for(const group of geometry.groups.filter(g=>g.materialIndex===2))for(let j=group.start;j<group.start+group.count;j++){const i=index.getX(j);if(Math.abs(p.getX(i)-edge)<1e-6)points.push(new Vector3().fromBufferAttribute(p,i).applyMatrix4(matrix).toArray().map(n=>n.toFixed(6)).join(','));}return [...new Set(points)].sort();};
    expect(seam(leftGeometry,.5,leftMatrix)).toEqual(seam(rightGeometry,-.5,rightMatrix));
    for(const geometry of [leftGeometry,rightGeometry]){geometry.dispose();}
...
it('convex/concave finishes close cut strips with one face owner and no separate modules',()=>{
  for(const cells of [box(3,4,3),box(3,4,3).filter(([x,,z])=>x!==2||z!==2)]){
    const result=generateDocument(createDocument(cells,42,'shop'));
    const finishes=result.placements.flatMap(p=>completeFaceAsset(p.faceAssetKey!).finishAssetKeys.map(assetKey=>({...p,assetKey,moduleId:p.finishIds![completeFaceAsset(p.faceAssetKey!).finishAssetKeys.indexOf(assetKey)]})));
    const geometry=finishes.flatMap(m=>TRIM_ASSETS[m.assetKey].boxes16.map(b=>({id:m.moduleId,box:faceBounds16(b,m.position2,m.orientationId)})));
    expect(finishes.some(m=>m.assetKey.includes('outer-'))).toBe(true);
    if(cells.length<36)expect(finishes.some(m=>m.assetKey.includes('inner-'))).toBe(true);
    ...
    expect(result.modules).toEqual([]);expect(result.placements).toHaveLength(result.surfaces.length);
    const terminals=finishes.filter(m=>m.moduleId.startsWith('terminal:'));expect(new Set(terminals.map(m=>m.moduleId)).size).toBe(terminals.length);
  }
});
it('a protected clearance rejects decoration atomically and shared prototypes are cached by asset key',()=>{
  const document=createDocument(box(3,4,3),42,'shop'),result=generateDocument(document),target=planFacadeTrims(document,result.surfaces,result.environment!.vertical!,result.environment!.preflight,new ReservationBook(result.environment!.spatial!.staticReservations)).finishes[0],clearanceBox=faceBounds16(TRIM_ASSETS[target.assetKey].bounds16,target.position2,target.orientationId);
  ...
  const plan=planFacadeTrims(document,result.surfaces,result.environment!.vertical!,result.environment!.preflight,book);
  expect(plan.finishes.some(m=>m.finishId===target.finishId)).toBe(false);expect(plan.traces.flatMap(t=>t.candidates).some(c=>c.conflictIds.includes('protected-clearance'))).toBe(true);
