...
import {exportDocument,loadDocument} from '../src/core/document';
...
it('catalog 4 documents keep authored legacy frame selections while upgrading',()=>{
  ...
  old.catalog.tiles=old.catalog.tiles.filter(t=>!t.assetKey.startsWith('facade.tower11-d-')&&!t.assetKey.startsWith('facade.city-')&&!t.assetKey.includes('streamline-c-')&&!t.assetKey.includes('curtain-b-')&&!t.assetKey.includes('ribbon-a-')&&!/^facade\.urban-(shop|office)-frame-/.test(t.assetKey));
