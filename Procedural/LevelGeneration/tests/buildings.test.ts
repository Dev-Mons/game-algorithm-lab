...
it("changes only the selected building and preserves volume and history", () => {
  ...
  const before = generateDocument(doc), next = setBuildingTheme(doc, "0,0,0", OFFICE_STYLE), after = generateDocument(next);
  expect(next.grid).toEqual(doc.grid);
  expect(after.placements.filter(p => p.faceId.startsWith("3,"))).toEqual(before.placements.filter(p => p.faceId.startsWith("3,")));
  expect(after.traces.find(t => t.componentId === "0,0,0" && t.facade)?.facade?.styleId).toBe("office");
...
it("merges by pre-edit volume, splits inherit, and equal volumes use numeric coordinate order", () => {
  ...
  expect(generateDocument(joined).traces.filter(t => t.facade).every(t => t.facade?.styleId === "office")).toBe(true);
