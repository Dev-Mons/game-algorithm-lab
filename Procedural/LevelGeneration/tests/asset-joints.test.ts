...
it('the shared roof panel covers its unit footprint once with outward normals',()=>{
 const parts=buildCraftedGeometry('crafted.roof'),g=parts.toNonIndexed(),p=g.getAttribute('position'),normal=g.getAttribute('normal');
 ...
 for(let i=0;i<normal.count;i++)expect(normal.getZ(i)).toBeGreaterThan(.99);g.dispose();parts.dispose();
});
