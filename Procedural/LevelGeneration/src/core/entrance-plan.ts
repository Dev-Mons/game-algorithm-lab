...
export function planEntrances(document:GenerationDocument,building:ResolvedBuildingMetadata,cells:Vec3[],surfaces:Surface[],spatial:SpatialAnalysis,book:ReservationBook,solid:BoundsIndex<string>,relations?:SceneRelationIndex):EntrancePlan {
  ...
  for(const run of runs){
    ...
    for(const width of [2,1] as const){if(faces.length<=2&&width===2)continue;
      for(let start=low;start+width<=high;start++){
        ...
        if(group.some(f=>columns.has(f.faceId))){trace.reasonCodes=['COLUMN_FACE'];continue;}
        ...
        if(!roadAccess){
          ...
          // reachability is separate from the presence of a building entrance.
          if(landing.some(c=>!search.nodes.has(cellId(c))))continue;
        }
        ...
        trace.metrics.pathLength=roadAccess?access[0].result.distanceCells!:0;trace.metrics.hardValid=true;
        trace.metrics.access=roadAccess?'road':'local';
        candidates.push({id,run,faces:group,landing,path:roadAccess?access[0].result:{reachable:false,path:[],reasonCodes:trace.reasonCodes},centerDistance2:Math.abs(dotU(group[0])+dotU(group[group.length-1])-dotU(faces[0])-dotU(faces[faces.length-1])),trace});
      }
    }
  }
  const frontage=new Set(candidates.filter(c=>c.path.reachable).flatMap(c=>c.faces.map(f=>f.faceId))),desiredCount=Math.max(candidates.length?1:0,desiredEntranceCount(frontage.size,cells.length,settings.entrances)),selected:Candidate[]=[],reservations:Reservation[]=[];
  ...
  while(selected.length<desiredCount&&remaining.length){
    ...
    if(!candidate.path.reachable&&selected.length){candidate.trace.reasonCodes=['LOCAL_ENTRANCE_ALREADY_PRESENT'];continue;}
