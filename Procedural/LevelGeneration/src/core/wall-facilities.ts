...
export function planWallFacilities(document:GenerationDocument,surfaces:readonly Surface[],vertical:VerticalPlan[],portals:ReadonlySet<string>,book:ReservationBook):WallFacilityPlan {
  ...
  for(const input of document.sceneInputs.objects.filter(o=>o.facilityKind)){
    ...
    if(!reason)for(let i=0;i<input.cells.length;i++){
      ...
      const asset=`wall-facility.${input.facilityKind}.${part(us[i],minU,maxU)}.${part(ys[i],minY,maxY)}`,descriptor=WALL_FACILITY_ASSETS[asset];assetKeys.push(asset);
      const heading=({PZ:0,PX:1,NZ:2,NX:3} as Record<string,Heading>)[input.direction],id=`facility:${input.id}:${face.faceId}`;
      const p:ScenePlacement={id,input,kind:'object',asset,center,size:[1,1,1],color:input.facilityKind==='elevator'?'#8fabb5':'#b8b0a0',context:'decorative-service-unverified',planId:`facility:${input.id}`,sourceRefs:[{kind:'object',id:input.id},{kind:'building',id:face.componentId}],yawQuarterTurns:heading};
