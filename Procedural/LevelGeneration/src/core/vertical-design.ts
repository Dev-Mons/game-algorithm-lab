...
export function planVertical(buildingId:string,cells:readonly Vec3[],surfaces:readonly Surface[],design:BuildingDesignV1,definition:BuildingStyle):VerticalPlan {
  ...
  const profile=resolveDesignProfile(style,design);
  ...
  return {buildingId,datumY,heightCells,bands,...(profile?{profile}:{}),...(mass?{mass,zones}:{}),alignment:{anchor:[...design.anchor],familyId:family.id,periodCells:family.periodCells,phaseByDirection},faceBands,
    ...
    traces:[{id:`vertical:${buildingId}`,ownerId:buildingId,ruleId:'banded-facade-v1',ruleVersion:String(style.version),sourceRefs:[{kind:'building',id:buildingId}],selectedIds:bands.map(b=>b.band),candidates:[{candidateId:'height-split',accepted:true,reasonCodes:allocation?.reasons??counts.reasonCodes,metrics:{height:heightCells,use:design.use,base:counts.base,body:counts.body,crown:counts.crown,requestedBase:counts.requestedBase,requestedCrown:counts.requestedCrown,family:family.id,...(profile?{program:profile.programId}: {})},conflictIds:[]}]}]};
}
