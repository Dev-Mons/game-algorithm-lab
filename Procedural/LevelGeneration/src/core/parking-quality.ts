...
export function parkingQuality(circulation:ParkingCirculationPlan,stalls:ParkingStall[],reservations:Reservation[],rejected:Record<string,number>,used:number,rejectedCells:Record<string,string>={},islands:Vec3[]=[]):{quality:ParkingQualityMetrics;unallocatedCells:Vec3[]}{
  ...
  const W=set(reservations.filter(r=>r.kind==='walk'||r.kind==='entrance').flatMap(r=>r.cells).filter(c=>P.has(cellId(c)))),S=set(stalls.flatMap(s=>s.cells)),I=set(islands);
  if([...S].some(k=>V.has(k)||W.has(k)||I.has(k))||[...I].some(k=>V.has(k)||W.has(k)))throw new Error('PARKING_QUALITY_OVERLAP');
  const unallocatedCells=circulation.eligibleCells.filter(c=>!V.has(cellId(c))&&!W.has(cellId(c))&&!S.has(cellId(c))&&!I.has(cellId(c))),reasons:Record<string,number>={};
