...
export function renderPlanInspector(host:HTMLElement,document:GenerationDocument,result:GenerationResult,source?:SourceRef){
  ...
  const sections:[string,unknown][]=[
    ...
    ['분석 · 계획',{fixtureCounters:environment?.fixtures?.counters,parking:environment?.parking?.filter(a=>a.areaId===source.id),entrances:environment?.entrances?.filter(e=>e.buildingId===source.id),parkingCirculation:environment?.parkingCirculation?.filter(a=>a.areaId===source.id),vertical:environment?.vertical?.filter(v=>v.buildingId===source.id),relations:environment?.spatial?.relations.filter(r=>same(r.from,source)||same(r.to,source)),diagnostics:environment?.spatial?.diagnostics.filter(d=>d.ownerId===source.id)}],
